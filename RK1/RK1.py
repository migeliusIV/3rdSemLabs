...
def main():
    ...
    for elem in files_tuple:
        if elem.name[0] == "A":
            print("File's id - {}. File's name - {}. Directory - {}".format(
                elem.file_id,
                elem.name,
                dir_tuple[elem.dir_id].name)
            )
    ...
    # V - 3
    print("\nTask V part 3")
    dirFiles_list = sorted(dirFiles_tuple, key = lambda dirFiles_tuple: dirFiles_tuple.file_id)
